<html lang="en-us" id="lock-page"><head>
		...
		<link rel="apple-touch-icon" sizes="76x76" href="{{ URL::asset('favicon.ico') }}">
		<link rel="apple-touch-icon" sizes="120x120" href="{{ URL::asset('favicon.ico') }}">
		<link rel="apple-touch-icon" sizes="152x152" href="{{ URL::asset('favicon.ico') }}">
	...
	<body class=" desktop-detected pace-done">
		<div id="main" role="main">
			<!-- MAIN CONTENT -->
			<form class="lockscreen animated flipInY" action="{{ URL::route('account-locked') }}" method="POST">
				{{ Form::token() }}
				<div class="logo">
					<h1 class="semi-bold"><img src="{{ URL::asset('favicon.ico') }}" alt=""> EMD</h1>
				</div>
				<div>
					<img src="{{ URL::asset('images/locked.png') }}" alt="" width="120" height="120">
					<div>
						<h1><i class="fa fa-user fa-3x text-muted air air-top-right hidden-mobile"></i>{{ Auth::user()->name }} <small><i class="fa fa-lock text-muted"></i> &nbsp;Locked</small></h1>
						<p class="text-muted">
							{{ Session::get('error', 'Please enter your password to unlock <br>your account.') }}
						</p>
						...
						<p class="no-margin margin-top-5">
							Logged as someone else? <a href="{{ URL::route('logout') }}"> Click here</a>
						</p>
					</div>
				</div>
		...
		<script> if (!window.jQuery) { document.write('<script src="{{ URL::asset('js/libs/jquery-2.0.2.min.js') }}"><\/script>');} </script><script src="{{ URL::asset('js/libs/jquery-2.0.2.min.js') }}"></script>
	    <script src="//ajax.googleapis.com/ajax/libs/jqueryui/1.10.3/jquery-ui.min.js"></script>
		<script> if (!window.jQuery.ui) { document.write('<script src="{{ URL::asset('js/libs/jquery-ui-1.10.3.min.js') }}"><\/script>');} </script><script src="{{ URL::asset('js/libs/jquery-ui-1.10.3.min.js') }}"></script>
